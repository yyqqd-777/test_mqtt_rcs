...
def build_mantis_move_command_set(robot_label: str, init_pos_x: int, init_pos_z: int, command_param: list, sCurve_profile: bool = False):
    ...
    for command_id, (target_pos_x, target_pos_z, vel_x, vel_z, acc_x, acc_z) in enumerate(command_param):
        commands.append(build_mantis_move_command(
            robot_label, set_id, str(command_id + 1), init_pos_x, init_pos_z, target_pos_x, target_pos_z, vel_x, vel_z, acc_x, acc_z, sCurve_profile))
...
def build_ant_home_command_set(
        robot_label: str,
        command_type: str,
        origin_offset_x: int = 0,
        origin_offset_y: int = 0,
        origin_orientation: int = 0
):
    topic = f"robot/commandSet/create/{robot_label}"
    set_id = f"SET-{datetime.now().isoformat()}"
    ...
    commands = [
        {
            ...
            "commandContent": {
                "robotCommandType": command_type,
                "originOffsetX": origin_offset_x,
                "originOffsetY": origin_offset_y,
                "originOrientation": origin_orientation
            },
            "expectedState": {
                "coordX": 0,
                "coordY": 0,
